...
— LegacyBot brings its own model and tools.
...
responses) worth separating from genuine `fail`s.
...
time. `--config` must be the same file the finding came from, which it is here since you only have the one.
